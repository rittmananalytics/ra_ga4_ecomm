    ...
    select
        ...
        array(
            select as struct
                ...
                cast(null as string) as creative_name,  -- Not in purchase
                cast(null as string) as creative_slot,  -- Not in purchase
                cast(null as float64) as discount,  -- Snowplow only
                cast(null as string) as index  -- Snowplow only
            from unnest(items) as item
        ) as items,

        -- Snowplow-specific ecommerce fields (null for GA4)
        cast(null as float64) as ecommerce_value,
        cast(null as string) as ecommerce_currency,
...
snowplow_purchase as (
    ...
    select
        ...
        -- transaction details (map Snowplow fields to GA4 structure)
        ecommerce.transaction_id,
        cast(null as float64) as purchase_revenue_in_usd,  -- Snowplow doesn't have USD-specific
        ecommerce.value as purchase_revenue,  -- Snowplow uses 'value' instead
        cast(ecommerce.tax as float64) as tax_value_in_usd,  -- Snowplow has tax but not USD-specific
        cast(ecommerce.shipping as float64) as shipping_value_in_usd,  -- Snowplow has shipping but not USD-specific
        cast(null as int64) as total_item_quantity,  -- Snowplow doesn't have this aggregate
        cast(null as int64) as unique_items,  -- Snowplow doesn't have this aggregate
        ...
        -- Snowplow-specific ecommerce fields
        ecommerce.value as ecommerce_value,
        ecommerce.currency as ecommerce_currency,
